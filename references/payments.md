...
2. 授权代付的授权可由平台发起，该功能仅为**授权代付合约**发起授权。不包含协议代扣合约的授权发起。
3. 发起授权代付申请的前置步骤：商户已开通过桥西进户。过桥西进户的开通流程详见

   1. 过桥西进户在商户入驻时由网商实时静默开通（仅此一种方式，商户无需单独申请）；开通后网商不会通知，平台也无法通过接口查询是否已开通。
4. 合约产生接口：
...
1. 页面表单填写说明
   ...
   1. 授权方：依次选择授权方公司/门店/供应商名称和授权方商户。支持关键词匹配。付款方商户名和网商商户号分别对应`AuthorizerInfo`中的`参与者名称participantName`和`参与者编号participantId`字段。
   
      1. 授权方公司/门店/供应商名称和商户号选择后，系统需通过商户号查询该商户是否已有生效的合约：
   ...
   4. 子场景和收款方：按场景聚类，分别填写每个场景的收款方信息。
      ...
      <callout emoji="🎈">
      1. 一个银行卡/支付宝账户不可同时出现在一个合约的多个子场景。即一个银行卡/支付宝账户只能在一个授权代付合约中出现一次。
      </callout>
      ...
      2. 子场景展示顺序及限制信息。
         ...
         5. 报销代发：`CLAIM`。该场景下单笔≤1万，所有收款方月度转账总和≤10万，最多可绑定200个收款方。
      3. 收款方信息填写规则
         ...
         1. 户名：必填，文本框输入，收款方账户名称。`participantName`字段。
         
            1. 银行卡：银行卡户名。
            2. 支付宝：支付宝户名。
         2. 账号：必填，文本框输入，收款方账号。`participantId`字段。
         
            1. 银行卡：银行卡卡号。
            2. 支付宝：支付宝登录号，手机号或邮箱。
         3. 类型：必填，下拉单选，对公对私类型，`bankCardHolderType`字段。
         ...
         5. 联行号：必填，对公银行卡联行号。`unitedBankCode`字段。
...
1. 查询区
   ...
   3. 授权方名称：对应列表中的授权方名称列的名称，文本框输入，支持关键词筛选（非完全匹配）。
      ...
      2. 门店：门店名称。
      3. 供应商：供应商名称。
   4. 授权方编码：对应列表中的授权方名称列的供应商编码，文本框输入，支持关键词筛选（非完全匹配）。
...
3. 列表区
   ...
   1. 协议代扣
      ...
      3. 授权方名称：网商商户号对应的公司/门店/供应商名称。
      4. 授权方编码：商户号对应的公司/门店/供应商编码。
      5. 授权方商户简称：
...
3. 授权方：依次展示{公司/门店/供应商名称}-{商户简称}-{网商商户号}，中间用「-」连接。
...
3. 授权方：依次展示{公司/门店/供应商名称}-{商户简称}-{网商商户号}，中间用「-」连接。网商商户号`participantId`和商户简称`participantName`通过`AuthorizerInfo`字段获取，公司/门店/供应商名称通过网商商户号反查获得。
...
11. 业务子场景、子场景限额和子场景收款方：按子场景聚类展示每个子场景下的授权方信息，业务子场景为`subSceneCode`字段的值，子场景限额为`agreementLimitInfo`字段的值，收款方为`receiptParticipantList`字段列表的值。
    ...
    3. 收款方信息展示：
       ...
       3. 类型：对公对私类型，`bankCardHolderType`字段。
          ...
          1. 对公：CORPORATION
          2. 对私：INDIVIDUAL
       4. 开户行：新增/修改授权代付合约时填写的开户行名称，无值时展示「-」。
...
1. 查询区
   ...
   4. 授权方名称：对应列表中的授权方名称列的名称，文本框输入，支持关键词筛选（非完全匹配）。
      ...
      2. 门店：门店名称。
      3. 供应商：供应商名称。
   5. 授权方编码：对应列表中的授权方公司/门店/供应商列的公司/门店/供应商编码，文本框输入，支持关键词筛选（非完全匹配）。
...
3. 列表区
   ...
   1. 协议代扣
      ...
      5. 授权方名称：
         ...
         1. 协议代扣：`1.1授权/解约成功通知接口<ant.mybank.merchantprod.merchant.arrangement.info.notify>`接口中`网商商户号MerchantId`字段值对应的公司/门店/供应商名称。
      6. 授权方编码：
      
         1. 协议代扣：`1.1授权/解约成功通知接口<ant.mybank.merchantprod.merchant.arrangement.info.notify>`接口中`网商商户号MerchantId`字段值对应的公司/门店/供应商编码。
      7. 授权方商户简称：
...
2. 数据来源接口
   ...
   2. 授权代付：2.2和2.3两个接口的参数字段一致。
      ...
      2. 发起付款：2.1授权协议支付申请接口<ant.mybank.bkcloudfunds.protocol.pay.apply>
      3. 付款结果通知：2.2.授权代付结果通知<ant.mybank.bkcloudfunds.trade.operate.result.notify>
      4. 查询付款结果：2.3.授权代付结果查询<ant.mybank.bkcloudfunds.trade.operate.result.query>
</callout>
...
3. 付款单列表区
   ...
   6. 收款方：
      ...
      1. 协议代扣：该付款单中的收款方主体和商户简称。对应网商`2.1协议授权代扣接口<ant.mybank.bkcloudfunds.order.``withhold``.apply>`接口中的`收款方ID PayeeId`字段值（`网商商户号`），通过该字段值查询到的收款方信息。
         ...
         3. 收款方为平台时：展示平台保证金户户名和平台保证金户账号（结算户，888开头）
   ...
   7. 备注：用户创建付款单时填写的备注信息
      ...
      2. 授权代付：对应网商`授权协议支付申请接口<ant.mybank.bkcloudfunds.protocol.pay.apply>`接口中的`备注Memo`字段值。
         ...
         1. 2.2通知接口和2.3查询接口也会返回该字段。
   8. 状态：按付款方式，展示对应付款单的状态。详见[协议代扣付款单状态](https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-JVLSdFbDhoXLS6xPtNVcpy9cnLd)和[授权代付付款单状态](https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-QVKPdNdTHoysw9xNlV0cWFbingh)。
   
      1. 处理中：网商已受理付款请求，正在处理中。
      2. 成功：付款执行成功，资金已划转。
      3. 失败：付款执行失败，资金未划转。展示错误信息。
   9. 完结时间：付款单网商侧完结时间，格式：yyyy-MM-dd HH:mm:ss。
...
4. 费用场景：付款记录数据产生时将费用场景名称写入记录数据，避免因用户修改费用场景名称带来的数据不一致问题。网商无此字段。
5. 业务场景：对应`2.1协议授权代扣`接口中的`业务场景Scene`字段值，一般为`品牌协议代扣支持冻结和账号场景`。
6. 付款金额：付款金额数字为2位小数，单位元，格式为`{x.xx}元`，对应网商`TotalAmount`字段值。
...
11. 付款方类型：商户。对应网商`PayerType`字段值`MERCHANT`。
12. 收款方：收款方主体信息和商户简称。对应网商`PayeeId`字段值网商商户对应的商户信息。

    1. 收款方为公司/门店/供应商时：展示{公司/门店/供应商名称}-{收款商户简称}-{收款方网商商户号}。
    2. 收款方为平台时：展示{平台保证金户户名}-{平台保证金户账号}。
    
       1. 平台保证金户户名：浙江可柔品牌管理有限公司。
       2. 平台保证金户账号：8888888815513823。
13. 收款方类型：商户或平台。对应网商`PayeeType`字段值。

    1. MERCHANT：商户
    2. PLATFORM：平台
14. 备注：发起付款申请时填写的备注信息，对应网商`Memo`字段值。
15. 指定付方使用信息：付款时出款账户的使用顺序，对应网商`PayerSpecifiedInfo`字段值。按字段值中的`priority`优先级数字从小到大依次排列。只展示出款账户，不展示具体金额。

    1. 可用子户冻结余额
    2. 可用子户解冻余额
    3. 交易保证金户余额
16. ~~订单类型：对应网商`OrderType`字段值。~~

    1. ~~多笔合并支付 ：`IMME_MERGE_SINGLE_PAY`~~
    2. ~~单笔支付：`MERGE_SINGLE_PAY`~~
17. 网商订单号：网商侧返回的订单号，对应网商`OrderNo`字段值。
18. 状态：付款单状态，详见[协议代扣付款单状态](https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-JVLSdFbDhoXLS6xPtNVcpy9cnLd)。
19. 交易完结时间：格式：yyyy-MM-dd HH:mm:ss，对应网商`交易完结时间FinishDate`字段值。
20. 错误码，对应网商`ErrorCode`字段值。状态为「FAIL」时出现。
21. 错误描述：对应网商`ErrorDesc`字段值，状态为「FAIL」时出现。
...
1. 付款方
   ...
   1. 选择付款方公司/门店/供应商：支持文本筛选的下拉框。用户输入文本后系统匹配公司/门店/供应商名称在下拉框展示可选择的公司/门店/供应商，用户点击后选择。
   2. 选择付款商户：支持文本筛选的下拉框。根据前面选择的公司/门店/供应商，下拉框展示该公司/门店/供应商关联的商户，如果公司/门店/供应商只有一个商户则默认展示该商户。
2. 付款方式：选择「对内付款（协议代扣）」。
...
3. 费用场景：单选费用场景。
   ...
   <callout emoji="🎈">
   1. 付款记录数据产生时将费用场景名称写入记录数据，避免因用户修改费用场景名称带来的数据不一致问题。
   </callout>
...
4. 收款方及金额
   ...
   5. 收款方信息填写：
      ...
      1. 选择收款方公司/门店/供应商：支持文本筛选的下拉框。用户输入文本后系统匹配公司/门店/供应商名称在下拉框展示可选择的公司/门店/供应商，用户点击后选择。
      2. 选择收款商户：支持文本筛选的下拉框。根据前面选择的公司/门店/供应商，下拉框展示该公司/门店/供应商关联的商户，如果公司/门店/供应商只有一个商户则默认展示该商户。
...
4. 收款方及金额
   ...
   3. 场景及限额信息：每个场景标题下方展示该场景的限额信息。
      ...
      5. 报销代发：该场景下单笔≤1万，所有收款方月度转账总和≤10万，最多可绑定200个收款方。
   4. 勾选收款方并填写付款金额。
...
2. 协议代扣退款弹窗：
   ...
   2. 付款方：带入原付款单付款方信息，格式为：{公司/门店/供应商名称}-{商户简称}。
   3. 收款方：带入原付款单收款方信息，格式为：{公司/门店/供应商名称}-{商户简称}。
...
1. 查询区
   ...
   3. 退款方
      ...
      1. 退款方名称：对应列表中的退款方列的退款方公司/门店/供应商名称，文本框输入，支持关键词筛选（非完全匹配）。
      2. 退款方商户简称：对应列表中的退款方列的退款方商户简称，文本框输入，支持关键词筛选（非完全匹配）。
   4. 收款方
   
      1. 收款方名称：对应列表中的收款方列的退款方公司/门店/供应商名称，文本框输入，支持关键词筛选（非完全匹配）。
...
3. 费用场景：付款记录数据产生时将费用场景名称写入记录数据，避免因用户修改费用场景名称带来的数据不一致问题。网商无此字段。
4. 付款金额：数字为2位小数，单位元，格式为`{x.xx}元`，对应网商协议代扣接口的`TotalAmount`字段值。
5. 状态：付款单状态，详见[协议代扣付款单状态](https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-JVLSdFbDhoXLS6xPtNVcpy9cnLd)。
6. 交易完结时间：原付款单完结时间，格式：`yyyy-MM-dd HH:mm:ss`，对应网商协议代扣通知和查询接口的`交易完结时间FinishDate`字段值。
7. 已退款金额：该付款单下所有状态为「成功」的退款单退款金额总和。
8. 退款在途金额：该付款单下所有状态为「处理中」的退款单退款金额总和。
9. 最大可退金额：原付款金额 - 已成功退款金额 - 处理中退款金额。
