...
1. 页面表单填写说明
   ...
   1. 授权方：依次选择授权方公司或供应商和授权方商户。支持关键词匹配。付款方商户名和网商商户号分别对应`AuthorizerInfo`中的`参与者名称participantName`和`参与者编号participantId`字段。
   
      1. 授权方供应商和商户号选择后，系统需通过商户号查询该商户是否已有生效的合约：
...
1. 查询区
   ...
   3. 授权方名称：对应列表中的授权方名称列的名称，文本框输入，支持关键词筛选（非完全匹配）。
      ...
      1. 公司：公司名称，含供应商和区域公司。
      2. 供应商：供应商名称。
   4. 授权方编码：对应列表中的授权方供应商列的供应商编码，文本框输入，支持关键词筛选（非完全匹配）。
...
3. 列表区
   ...
   1. 协议代扣
      ...
      3. 授权方名称：网商商户号对应的公司或供应商名称。
      4. 授权方编码：商户号对应的公司或供应商编码。
      5. 授权方商户简称：
...
3. 授权方：依次展示{公司或供应商名称}-{商户简称}-{网商商户号}，中间用「-」连接。
...
3. 授权方：依次展示{公司或供应商名称}-{商户简称}-{网商商户号}，中间用「-」连接。网商商户号`participantId`和商户简称`participantName`通过`AuthorizerInfo`字段获取，供应商名称通过网商商户号反查获得。
...
1. 查询区
   ...
   4. 授权方名称：对应列表中的授权方名称列的名称，文本框输入，支持关键词筛选（非完全匹配）。
      ...
      1. 公司：公司名称，含供应商和区域公司。
      2. 供应商：供应商名称。
   5. 授权方编码：对应列表中的授权方供应商列的供应商编码，文本框输入，支持关键词筛选（非完全匹配）。
   ...
   2. 授权代付
      ...
      5. 授权方名称：
         ...
         1. 授权代付：
            ...
            1. 签约、修改：`1.4. 查询申请单 目前状态<ant.mybank.bkcloudfunds.protocol.sign.apply.auerv>`接口中`授权方AuthorizerInfo`字段值中商户号对应的公司或供应商名称。
            2. 解约：`1.7.授权合同变更通知<ant.mybank.bkcloudfunds.protocol.sign.notify>`接口中`授权方AuthorizerInfo`字段值中商户号对应的公司或供应商名称。
      6. 授权方编码：
      
         1. 授权代付：
         
            1. 签约、修改：`1.4. 查询申请单 目前状态<ant.mybank.bkcloudfunds.protocol.sign.apply.auerv>`接口中`授权方AuthorizerInfo`字段值中商户号对应的公司或供应商编码。
            2. 解约：`1.7.授权合同变更通知<ant.mybank.bkcloudfunds.protocol.sign.notify>`接口中`授权方AuthorizerInfo`字段值中商户号对应的公司或供应商编码。
      7. 授权方商户简称：
...
6. 授权方：即该申请单发起方。
   ...
   1. 签约、解约：签约和解约申请单由公司或供应商发起。{公司或供应商名称}-{商户简称}-{网商商户号}。
...
13. 授权方：`1.4. 查询申请单 目前状态<ant.mybank.bkcloudfunds.protocol.sign.apply.auerv>`接口中`授权方AuthorizerInfo`字段的值。
    ...
    1. 授权方是公司或供应商：{公司或供应商名称}-{商户简称}-{网商商户号}
...
3. 付款单列表区
   ...
   5. 收款方：
      ...
      1. 协议代扣：该付款单中的收款方主体和商户简称。对应网商`2.1协议授权代扣接口<ant.mybank.bkcloudfunds.order.witHHold.apply>`接口中的`收款方ID PayeeId`字段值（`网商商户号`），通过该字段值查询到的收款方信息。
         ...
         3. 收款方为供应商时：展示供应商名称和付款商户简称。
         4. 收款方为平台时：展示平台保证金户户名和平台保证金户账号（结算户，888开头）
...
1. 填写批量付款表格并上传
   ...
   2. 下载空白模板：根据前面选择的付款方式，调用浏览器下载对应的空白模板。
      ...
      1. 对内付款（协议代扣）表格关键内容
         ...
         2. 请勿修改表格结构。
         3. 红色字段必填，黑色字段选填
         4. 付款方公司/供应商名称、付款方商户简称：输入名称后自动从「商户信息表」中匹配，须确保公司/供应商名称、商户简称和商户信息表中的对应关系一致。
         5. 付款方商户号：选择好付款方商户简称后系统自动从「商户信息表」中匹配对应的付款方商户号，请勿手动修改。
         6. 付款金额：即转账金额，单位：元，保留小数点后2位。
         7. 收款方公司/供应商名称、收款方商户简称：输入名称后自动从「商户信息表」中匹配，须确保公司/供应商名称、商户简称和商户信息表中的对应关系一致。
...
1. 付款方
   ...
   1. 选择付款方供应商：支持文本筛选的下拉框。用户输入文本后系统匹配供应商名称在下拉框展示可选择的供应商，用户点击后选择。
   2. 选择付款商户：支持文本筛选的下拉框。根据前面选择的供应商，下拉框展示该供应商关联的商户，如果供应商只有一个商户则默认展示该商户。
2. 付款方式：选择「对内付款（协议代扣）」。
...
4. 收款方及金额
   ...
   5. 收款方信息填写：
      ...
      1. 选择收款方供应商：支持文本筛选的下拉框。用户输入文本后系统匹配供应商名称在下拉框展示可选择的供应商，用户点击后选择。
      2. 选择收款商户：支持文本筛选的下拉框。根据前面选择的供应商，下拉框展示该供应商关联的商户，如果供应商只有一个商户则默认展示该商户。
...
2. 协议代扣退款弹窗：
   ...
   2. 付款方：带入原付款单付款方信息，格式为：{公司或供应商名称}-{商户简称}。
   3. 收款方：带入原付款单收款方信息，格式为：{公司或供应商名称}-{商户简称}。
...
1. 查询区
   ...
   3. 退款方
      ...
      1. 退款方名称：对应列表中的退款方列的退款方公司或供应商名称，文本框输入，支持关键词筛选（非完全匹配）。
      2. 退款方商户简称：对应列表中的退款方列的退款方商户简称，文本框输入，支持关键词筛选（非完全匹配）。
   4. 收款方
   
      1. 收款方名称：对应列表中的收款方列的退款方公司或供应商名称，文本框输入，支持关键词筛选（非完全匹配）。
...
3. 列表区
   ...
   3. 退款方：该退款单中的退款方主体和商户信息。退款方即原付款单中的收款方。对应网商退款申请接口`<ant.mybank.bkcloudfunds.protocol.witHHold.refund.apply>`中的`ParticipantId`字段值（即原付款单的`PayeeId`），通过该字段值查询到的退款方信息。退款通知接口和退款查询接口也会返回该字段。
      ...
      2. 退款方为供应商时：展示{供应商名称}-{商户简称}。
      3. 退款方为平台时：展示{平台保证金户户名}-{平台保证金户账号}。
