...
3. 补充：上述 6 个个体工商户营业执照为安徽地区门店共开通了 100+ 个收银 POS，POS 与门店的对应关系不能凭执照自动映射，需要通过清算订单模块的「POS 绑定商户」功能人工建立绑定关系。
4. 资金进入品牌方银行卡后，需要品牌方财务人员手动将资金转入网商为品牌方开具的[平台清算专户](https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-AY8BdpSProdaQ8xkG3CcEOOTnQb)。
...
3. 解释：
   ...
   1. `{品牌名}`：品牌名缩写，可柔品牌的所有供应商统一为「CARE」开头。
...
2. 状态说明
   ...
   1. 未入驻：商户在本系统（「新增商户入驻」按钮）刚创建时的初始状态。
...
2. 网商云资金小程序的「可用子户余额提现」入口支持通过邮件向网商银行申请打开或屏蔽，邮件操作规范如下。
3. 需特别注意：

   1. 该屏蔽仅针对「可用子户余额提现」；
   2. 「充值余额提现」（交易保证金户余额，本质充值退款）不受影响、不屏蔽。
</callout>

| 项目 | 内容 |
|-|-|
| 收件邮箱 | `pub_mybkyzj@mybank.cn` |
| 发件邮箱 | 品牌协议预留邮箱 |
| 邮件标题 | {公司全称} + 小程序提现按钮打开/屏蔽申请 |
| 邮件正文 | {公司全称} + 申请打开/屏蔽云资金小程序提现按钮  <br/>机构号（isv，2022 开头）：{机构号} |

1. 邮件内容

   <callout emoji="📬">
   收件邮箱：pub_mybkyzj@mybank.cn 
   发件邮箱：zhejiangkerou@126.com
   标题：浙江可柔品牌管理有限公司+小程序提现按钮屏蔽申请
   正文：
   浙江可柔品牌管理有限公司 申请屏蔽云资金小程序提现按钮 
   机构号：202211000000000004381
   </callout>
...
|-|-|-|-|-|-|
| **租金（原房租）** | RENT | 2.5 万 | 25 万 | 5 | 首次签约：需商户法人签约授权  <br/>信息变更：需商户法人签约确认 |
| **水电物业费** | FEE | 2 万 | 10 万 | 10 | 首次签约：需商户法人签约授权  <br/>信息变更：需商户法人签约确认 |
