...
<synced-source><callout emoji="🎈"><p>完整清算操作流程：</p><ol><li seq="1">收钱吧/拉卡拉 POS 收款 → 资金落在华夏银行卡 → 资金归集（财务人员手动转账到9903开头的网商平台清算专户，详见<a href="https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-HWrjdN3TKo8AMQx8hv2cHOAInMg">资金归集&amp;平台清算专户</a>） <p text-indent="1">                                   → 订单同步/导入至系统→ 系统侧完成清算订单上送</p><p>                                                                                                                                  → 清算文件上传→ 清算批次上报</p><p>                                                                                                                                                                                         → 网商按批次处理清算批次任务→ 返回清算结果</p><p>                                                                                                                                                                                                                                                                   → 清算成功</p><p>                                                                                                                                                                                                                                                                   →清算失败→ 重新执行生成清算文件及后续清算流程</p></li></ol></callout></synced-source>

<callout emoji="🎈">
1. POS收银的钱进入到POS绑定华夏银行卡中。
2. 在正式发起清算时需要财务人员手动将资金转入到网商平台清算专户中，详见[平台清算专户信息](https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-VEladFabmoaHdRxqnhEcB9K2nvh)，且需确保品牌清算专户余额大于清算批次对应文件中的实际金额。
...
1. 查询区
   ...
   9. 商户：支持用商户简称和商户编码筛选。
      ...
      2. 商户编码：列表中未展示，匹配商户对应的商户编码，文本框输入，支持关键词筛选（非完全匹配）。
2. 工具区

   1. 上送订单：
   
      <synced-source><callout emoji="🧨"><ol><li seq="1">用户需先筛选「未上送、上送失败」状态的订单，然后在列表中勾选，勾选后点击该按钮继续操作。</li><li>订单上送需同时满足以下2个条件：<ol><li seq="1">订单状态为「未上送、上送失败」。</li><li>订单所属POS机已绑定商户。</li></ol></li><li>支付单和退款单的上送流程不同。<ol><li seq="1">支付单（正向单）：<ol><li seq="1">判断订单状态应为「未上送、上送失败」。</li><li>上送操作：<ol><li seq="1">调用网商银行<code>补单创单接口&lt; ant.mybank.bkcloudfunds.bill.pay&gt;</code> 执行订单上送操作，接口调用详见<a href="https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-GWrcdunDroacYsxzhGOc3EVan9e">订单上送接口调用</a>。</li><li>上送后网商返回<code>{网商订单号}</code>视作上送成功</li></ol></li></ol></li><li>退款单（逆向单）：<ol><li seq="1">判断订单状态应为「未上送、上送失败」。</li><li>上送操作：<ol><li seq="1">查询该退款单关联的原支付单的「网商订单号」。<ol><li seq="1">原支付单的「网商订单号」不存在，即原支付单未上送成功：退款单上送会失败。</li><li>原支付单的「网商订单号」存在，即原支付单已上送成功：退款单上送也会成功。</li></ol></li></ol></li></ol></li></ol></li></ol></callout></synced-source>
   ...
   2. 生成清算文件
      ...
      5. 如勾选订单无异常（仅包含「已上送」且所有订单批次号均为「-」和「清算失败」订单）：弹窗提示。
      
         1. 弹窗内容：「清算文件将在后台生成，稍后可前往「清算文件」查看。」
         2. 其中「清算文件」用蓝色字体路由至「清算文件」页面。
   3. 导入订单
   
      <synced-source><callout emoji="🎈"><ol><li seq="1">仅支持导入拉卡拉账单，请勿导入其他收银机构的账单。</li><li>账单唯一编号：拉卡拉账单<code>{交易流水号}</code>。</li></ol></callout></synced-source>
                  ...
                  2. 下载规则
                     ...
                     2. 文件内容表头：在原导入模板的表头后面增加结果列：
                        ...
                        1. 结果
                           ...
                           1. 成功：表示该列数据导入失败，即校验通过，已新增至订单列表。
                           2. 失败：表示该列数据导入失败，即该列数据未新增至订单列表。展示状态和失败原因，以下为部分失败情况，其他未列举情况根据系统提示展示。
                           
                              1. 数据错误：展示为「失败，订单数据错误」。
                              2. 数据重复：展示为「失败，订单已存在，请检查订单数据」。订单唯一性通过拉卡拉账单`{交易流水号}`字段识别。
                              3. 系统原因导致失败：展示为「失败，服务异常，请稍后重试」。
      3. 拉卡拉账单解析字段映射
      
         1. 详见[拉卡拉账单](https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-Rwy1dA2SRonuzVxE6SRcwXuVn7f)
3. 订单列表区
   ...
   11. 网商订单号：调用网商`补单创单接口< ant.mybank.bkcloudfunds.bill.pay>`接口上送订单成功后网商返回字段`{OrderNo}`的值。有该值则代表订单上送成功，未上送订单无该值。
       ...
       1. 退款单不需要调用网商接口上送订单，但是需要填写网商订单号，退款单的网商订单号为其对应的支付单的网商订单号。上送退款时的校验逻辑见订单上送模块。
   12. 批次号：勾选订单并成功生成清算文件后系统产生，具体逻辑详见生成清算文件。

   ##### 拉卡拉订单
   
   1. 订单编号：支付单仅展示支付订单号，退款单展示退款订单号+其关联的支付单号。
   
      1. 支付单：仅展示支付单订单号。对应拉卡拉账单`{交易流水号}`
      2. 退款单：同时展示退款单号和关联的支付单号。
      
         1. 退款单号：对应拉卡拉账单`{交易流水号}`
         2. 关联支付单号：对应拉卡拉账单`{原交易流水号}`
   2. 收银机构：根据订单来源区分，分为`收钱吧`和`拉卡拉`。拉卡拉订单显示为：「拉卡拉」。
   3. POS名称：订单所属的POS机名称，对应拉卡拉对账单`{网点名称}`。
   4. 实收金额（元）：拉卡拉对账单`{结算金额}`。即拉卡拉扣除其手续费后的品牌方到账金额。
   ...
   11. 网商订单号：调用网商`补单创单接口< ant.mybank.bkcloudfunds.bill.pay>`接口上送订单成功后网商返回字段`{OrderNo}`的值。有该值则代表订单上送成功，未上送订单无该值。
       ...
       1. 退款单不需要调用网商接口上送订单，但是在生成清算文件时需要填写网商订单号，退款单的网商订单号为其对应的支付单的网商订单号。上送退款时的校验逻辑见订单上送模块。
   12. 批次号：勾选订单并成功生成清算文件后系统产生，具体逻辑详见生成清算文件。
   ...
   2. 操作说明
      ...
      1. 上送：系统先判断该订单所属POS是否已绑定商户。
         ...
         1. 已绑定：调用网商`补单创单接口< ant.mybank.bkcloudfunds.bill.pay>`上送订单信息。
         
            1. 上送成功：toast提示：订单上送成功，请刷新列表查看。
            2. 上送失败：订单上送失败，请稍后重试。
         2. 未绑定：toast提示：该订单所属 POS 机未绑定商户，请先在「POS设置」中绑定商户后再上送。

### 订单汇总（按POS/门店）

<callout emoji="💡">
1. 提供按POS和按门店两个维度的订单聚合视图，支持批量上送订单操作。
2. 相比清算订单明细页，汇总页提供两个维度：按POS和按门店为粒度聚合订单，两个页面共享相同的底层数据（收钱吧/拉卡拉对账单）和上送接口（补单创单接口），但操作粒度不同：清算订单页按单笔订单上送，订单汇总页按POS维度批量上送。
3. 前置依赖：POS设置（POS-商户绑定关系）、清算订单（订单数据来源）、补单创单接口（上送操作底层接口）。
</callout>

1. 查询区

   1. 支付时间：对应列表中的支付时间列，提供全部、近60天、近30天、近7天、昨天（默认选中）快捷筛选，也支持通过下拉日期自定义时间段筛选（开始时间至结束时间）。
   2. 收银机构：对应列表中的收银机构列，下拉单选，选项包括全部、拉卡拉、收钱吧。
   ...
   5. 商户：支持用商户简称和商户编码筛选。
      ...
      1. 商户简称：对应列表中商户列的商户简称，文本框输入，支持关键词筛选（非完全匹配）。
      2. 商户编码：对应列表中商户列的商户编码，文本框输入，支持关键词筛选（非完全匹配）。
   6. 操作按钮：
   
      1. 筛选：点击后按当前筛选条件刷新列表。
      2. 清除筛选：点击后清空所有筛选条件，恢复默认列表。
2. 工具区

   1. 计算待清算金额用户勾选列表中的POS/门店行后，点击按钮弹出弹出待清算金额弹窗，展示选中POS/门店的待清算金额合计。「待清算订单金额合计」面板展示内容：操作按钮：
   
      <callout emoji="💡">
      待清算订单金额 = 「未上送、上送失败、已上送、清算失败」四状态订单的实收取金额合计（`支付单的实收金额-退款单的实收金额`）。
      待清算订单包含「未上送」「上送失败」「已上送」「清算失败」四种状态。
      「已上送」订单分为「已上送未生成清算文件」和「已上送已生成清算文件」。
      
      未生成清算文件的订单需要先生成清算文件再创建清算批次才能发起清算。
      
      已生成清算文件的订单可用清算文件创建清算批次发起清算。
      </callout>
   
      按订单状态维度展示订单合计金额。
   
      其中已上送订单分拆为已上送未生成清算文件和已上送已生成清算文件，判断依据：
      
      已上送未生成清算文件：状态=「已上送」且清算批次号=「-」。
      
      已上送已生成清算文件：状态=「已上送」且清算批次号≠「-」。
   
      合计：计算前面列表中的金额数字求和，2位小数，单位：元，格式为`{x.xx}元`
      ...
      <callout emoji="💡">
      注意：待清算金额面板仅做金额展示和规则说明，不触发任何业务流程。上送操作通过列表操作列的「上送」按钮或工具区的「上送订单」按钮触发。
      </callout>
   2. 上送订单
   
      <callout emoji="💡">
      1. 用户需先勾选POS或门店行，然后点击该按钮继续操作。
      2. 订单上送需同时满足以下2个条件：
      ...
      3. 支付单和退款单的上送流程不同。
         ...
         1. 支付单（正向单）：
            ...
            2. 上送操作：
               ...
               1. 调用网商银行**补单创单接口<ant.mybank.bkcloudfunds.bill.pay>**执行订单上送操作。
         ...
         2. 退款单（逆向单）：
            ...
            2. 上送操作：
               ...
               1. 查询该退款单关联的原支付单的「网商订单号」。
                  ...
                  2. 原支付单的「网商订单号」存在，即原支付单已上送成功：退款单上送也会成功。
      </callout>
   
      1. 未勾选任何门店行：toast提示：「请先选择POS或门店，然后再执行当前操作。」
      2. 没有可上送的订单：toast提示：「所选POS或门店没有可上送的订单。」
      3. 如勾选订单中存在不符合上送条件的订单（包含非「未上送、上送失败」状态的订单，或所属 POS 机未绑定商户的订单）：弹窗提醒。
      
         1. 弹窗内容：
         
            <callout emoji="🎈">
            按实际校验结果，列出勾选订单中所有不满足上送条件的情况（可能一条或多条并列）
            </callout>
         
            1. 已选择的订单中包含非「未上送、上送失败」状态的订单。
            2. 已选择的订单中包含所属 POS 机未绑定商户的订单。
            3.  \- 点击「继续上送」按钮将过滤不符合上送条件的订单，仅对「未上送、上送失败」状态且所属 POS 机已绑定商户的订单执行上送操作。
         2. 操作：
            ...
            2. 继续上送：系统依次执行以下动作：
               ...
               1. 对已选择订单中状态非「未上送、上送失败」的订单进行过滤。
               2. 对已选择订单中所属 POS 机未绑定商户的订单进行过滤。
               3. 仅对「未上送、上送失败」状态且所属 POS 机已绑定商户的订单执行上送操作：
               
                  1. 支付单：调用网商补单创单接口执行上送操作，根据网商返回的上送结果变更支付单状态。
                  2. 退款单：查询其关联的支付单的网商订单号，将该支付单的网商订单号同步为该退款单的网商订单号后将退款单状态变更为「已上送」。如未查询到支付单的网商订单号（支付单不存在/支付单未上送成功），则退款单状态变更为「上送失败」。
      4. 如用户勾选门店行仅包含「未上送、上送失败」的支付单和退款单，且订单所属POS均已绑定商户，系统依次执行以下动作：
      
         1. 支付单：调用网商补单创单接口<ant.mybank.bkcloudfunds.bill.pay>执行上送操作，根据网商返回的上送结果变更支付单状态。
         2. 退款单：查询其关联的支付单的网商订单号，将该支付单的网商订单号同步为该退款单的网商订单号，成功后退款单状态变更为「已上送」。如未查询到支付单的网商订单号（支付单不存在/支付单未上送成功），则退款单状态变更为「上送失败」。
         3. 弹窗提示：「订单上送将在后台执行，稍后可刷新列表查看上送状态。」
   ...
   3. 生成清算文件
      ...
      <callout emoji="💡">
      1. 用户需先勾选POS/门店行，然后点击该按钮继续操作。
      2. 用于生成清算文件的订单（含支付单和退款单）需满足以下条件任一：
      
         1. 订单（含支付单和退款）状态为「已上送」（网商订单号不为空）且批次号为「-」。
         2. 订单（含支付单和退款）状态为「清算失败」。「清算失败」的订单重新生成清算文件时，其批次号会被重置。
      3. 清算文件生成时系统自动生成批次号，详见[清算批次号生成规则](https://bcno92iwldd2.feishu.cn/docx/P4eedyyrhoWE5HxC1AtcCPvtnWc#share-VwbQdI6bso0i6uxqP3IcMO8Vnyc)。
      </callout>
   
      1. 未勾选任何门店行：toast提示：「请先选择POS或门店，然后再执行当前操作。」
      2. 没有可生成清算文件的订单：toast提示：「所选POS或门店没有可生成清算文件的订单。」
      3. 如勾选订单中存在不符合生成清算文件条件的订单（包含非「已上送」和「清算失败」状态的订单，或「已上送」状态但批次号不为「-」的订单）：弹窗提醒。
      
         <callout emoji="🎈">
         按实际校验结果，列出勾选订单中所有不满足生成条件的情况（可能一条或多条并列）：
         </callout>
         ...
         1. 弹窗内容
            ...
            1. 已选择的订单中包含非「已上送」和「清算失败」状态的订单。
            2. 已选择的订单中包含「已上送」状态且已有清算批次的订单。
            3.  \- 点击「继续生成」按钮将过滤不符合条件的订单，仅对「清算失败」和「已上送」状态且批次号为「-」的订单生成清算文件。
         2. 操作：
            ...
            2. 继续生成：系统依次执行以下动作：
               ...
               2. 对已选择订单中「已上送」状态且批次号不为「-」的订单进行过滤。
               3. 仅对「清算失败」和「已上送」状态且批次号为「-」的订单按清算文件的要求生成补单清算文件（交易明细文件）。
      4. 如勾选订单无异常（仅包含「已上送」且所有订单批次号均为「-」和「清算失败」订单）：弹窗提示。
...
3. 列表区
   ...
   <callout emoji="🎈">
   1. 列表区提供两个Tab切换聚合维度：**按POS**（默认）和**按门店**。
   2. 默认展示全部POS或门店（即使该POS或门店暂无待清算订单）。
   3. 默认排序与POS列表和门店列表的排序一致。
   4. 切换 Tab 时清空勾选 。
   </callout>
   ...
   1. 按POS
      ...
      <callout emoji="🎈">
      以POS机为聚合维度，每个POS机一行，展示该POS下所有待清算订单的汇总信息。
      包含收钱吧和拉卡拉所有的POS。
      </callout>
   
      1. 复选框：勾选POS行，用于批量上送操作。
      2. POS名称：拉卡拉/收钱吧的POS机名称，也是本列表的聚合维度。
      3. 收银机构：清算订单的收银机构，即拉卡拉/收钱吧。
      4. 待清算金额(元)：计算字段。该POS下所有待清算订单的实收金额总和（支付单-退款单（正值））。
      
         1. 待清算订单包含「未上送」「上送失败」「已上送」「清算失败」四种状态。待清算金额=支付金额-退款金额。
      5. 清算中金额(元)：计算字段。该POS下所有状态为「清算中」的订单的实收金额总和（支付单-退款单（正值））。
      6. 渠道类型：本系统定义字段，0：门店订单，1：微信小程序订单。当前收钱吧订单全部为门店订单，为未来支持微信小程序订单预留。
      7. 区域：POS关联商户所属门店的区域，如：安徽公司。
      8. 门店
      ...
      9. 商户
         ...
         1. 商户简称：POS机关联商户的商户简称。无数据时展示为「-」。
         2. 商户编码：商户的商户编码。无数据时展示为「-」。
      10. 操作：
      
          1. 「上送」按钮，蓝色文字链接。按钮逻辑同工具区的「上送订单」按钮。
   2. 按门店
   
      <callout emoji="🎈">
      1. 以门店为聚合维度，每个门店一行，展示该门店下所有商户绑定的POS机的待清算订单的汇总信息。一个门店下可能包含多个商户的多个POS机。
      </callout>
   
      1. 复选框：勾选门店行，用于批量上送操作。
      2. 门店
      
         1. 门店名称：从三联同步过来的门店名称。也是本列表的聚合维度。
         2. 门店编码：门店的门店编码。
      3. 收银机构：清算订单的收银机构，即拉卡拉/收钱吧。
      4. 待清算金额(元)：计算字段。该POS下所有待清算订单的实收金额总和（支付单-退款单（正值））。
...
2. 标档文件：
   ...
   1. 标档文件的作用：证明清算文件已上传成功。
   2. 标档文件上传路径：/upload/\${isvOrgId}/\${acquireId}/\${batchNo}/finish\_\${batchNo}.txt
   3. 标档文件上传至网商SFTP服务器的文件名：「finish\_\${batchNo}.txt」，其中{batchNo}为清算批次号。
   4. 标档文件为-- 空文件。
3. ⚠️ 每天最多上传 50 次清算文件，上传失败不占用次数。
4. 每个清算文件（清算批次）中「实际金额」须＞0。
</callout>
...
2. 生产环境
   ...
   1. SFTP账号：202211000000000004381
   2. 密码：7T9JWFX4Zn
   3. 文件下载地及文件命名方式： 
...
3. 列表区
   ...
   3. 实际金额：金额数字为2位小数，单位元，格式为`{x.xx}元`，该清算文件中`全部支付订单的实际金额`减去`全部退款订单的实际金额`。网商清算时以该金额为准。
   ...
   8. 操作说明：
      ...
      1. 上传：点击后系统依次上传`清算文件`和`标档文件`。两个文件都上传成功时才视为清算文件上传成功，有一个失败则视为清算文件上传失败。
      
         1. 标档文件：文件名与
         2. 上传成功：toast提示：清算文件上传成功，请刷新列表查看。
         3. 上传失败：toast提示：清算文件上传失败，请稍后重试。
...
3. 操作：
   ...
   3. 保存并上报：系统先保存批次信息，然后调用`创建批次接口<ant.mybank.bkcloudbatch.batch.create>`发起清算。
      ...
      2. 上报成功：清算批次上报成功，状态变更为「清算中」，页面跳转至上报成功页。页面内容：
         ...
         3. 批次实例号：创建批次接口返回的网商实例号`BatchInstanceId`
         4. 按钮操作：
...
3. 列表区
   ...
   4. 批次实例号：调用网商`创建批次接口<ant.mybank.bkcloudbatch.batch.create>`成功后网商返回的`BatchInstanceId`，和批次号一一对应。
...
4. 清算文件：清算文件名，从清算文件带入，跟批次号同名。
5. 备注：选填，用户可填写该批次的备注内容。
6. 批次实例号：网商`创建批次接口<ant.mybank.bkcloudbatch.batch.create>`成功后返回的`BatchInstanceId`。
7. 状态：该批次的清算状态。
8. 失败原因：
...
<callout emoji="🎈">
1. 清算批次提交后，网商会按商户维度聚合，然后进行清算操作。如果商户有退款订单，且商户可用余额少于退款金额时，网商会从品牌方 8888 平台保证金户（结算户）帮商户垫付该笔订单退款，即发生扣保。
...
3. 然后品牌方需发起退保申请以完成退保，该过程会产生一条退保单，与扣保单一一对应。
   ...
   1. 退保即资金从商户可用余额退回至品牌方 8888 平台保证金户的动作。
...
1. 扣保记录下文也称为扣保单。
2. 收到网商的扣保通知后系统创建扣保单。
3. 完整的扣保单（含退保成功）数据来源包括补单保证金扣除通知接口，发起退保申请接口，退款结果查询接口，和补单保证金退回通知接口的数据。
</callout>
...
2. 工具区
   ...
   1. 更新退保状态：点击后调用`退款结果查询接口``<ant.mybank.bkcloudbatch.stmt.deposit.return.query>`查询扣保单最新状态。
...
3. 列表区
   ...
   4. 退保申请提交后可通过查询接口获取最新退保状态。
      ...
      1. 查询接口：`退款结果查询接口``<ant.mybank.bkcloudbatch.stmt.deposit.return.query>`
   ...
   5. 退保流水号：发起退保申请时品牌方产生。对应网商`退保申请`和退款结果查询接口的`OutTradeNo`字段，`退保结果通知`接口的`RefundDepositOutTradeNo`字段值。
   ...
   8. 失败原因：退保状态为「`FAIL`失败」时出现。对应退款结果查询接口`Msg`字段值。
   9. 操作
